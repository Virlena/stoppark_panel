...
class FixedTariff(Tariff):
    ...
    (2, 8, 20, 57, 57)
    >>> tariff.calc(datetime(2013,12,13,8,0,0), datetime(2013,12,13,9,10,0))
    (0, 1, 10, 1, 1)
    >>> tariff.calc(datetime(2013,11,30,8,0,0), datetime(2013,12,1,9,0,0))
    (1, 1, 0, 25, 25)
    >>> tariff.calc(datetime(2008,2,28,16,0,0), datetime(2008,2,29,16,10,0))
    ...
    >>> tariff = Tariff.create(['1', 'Час 5 грн.', '1', '1', '5', 'None', '100', 'None'])
    >>> tariff.calc(datetime(2013,10,26,8,0,0), datetime(2013,10,28,16,20,0))
    (2, 8, 20, 57, 245)
    >>> tariff.calc(datetime(2013,12,13,8,0,0), datetime(2013,12,13,9,10,0))
    (0, 1, 10, 1, 5)
    ...
    >>> tariff = Tariff.create(['1', 'Час 1 грн.', '1', '3', '1', '09:00', 'None', 'None'])

    #mounth it is only 30 day - not calendar (31 or 28 or 29)
    >>> tariff.calc(datetime(2014,1,1,9,0,0), datetime(2014,2,1,9,0,0))
    (31, 0, 0, 2, 2)
    >>> tariff.calc(datetime(2014,4,1,8,0,0), datetime(2014,5,1,9,30,0))
    (30, 1, 30, 2, 2)
    >>> tariff.calc(datetime(2014,2,1,9,0,0), datetime(2014,3,1,9,0,0))
    (28, 0, 0, 1, 1)
    >>> tariff.calc(datetime(2014,2,1,8,0,0), datetime(2014,3,1,9,0,0))
    (28, 1, 0, 1, 1)
    >>> tariff.calc(datetime(2013,10,26,8,0,0), datetime(2013,10,28,16,20,0))
    (2, 8, 20, 1, 1)
    >>> tariff.calc(datetime(2013,12,13,8,0,0), datetime(2013,12,13,9,10,0))
    (0, 1, 10, 1, 1)
    >>> tariff.calc(datetime(2013,10,30,8,0,0), datetime(2013,12,30,9,30,0))
    (61, 1, 30, 3, 3)
    >>> tariff.calc(datetime(2008,1,1,16,0,0), datetime(2008,3,1,16,10,0))
    (60, 0, 10, 2, 2)
    >>> tariff.calc(datetime(2013,10,31,8,0,0), datetime(2014,1,10,9,5,0))
    (71, 1, 5, 3, 3)
    >>> tariff.calc(datetime(2013,12,9,9,0,0), datetime(2014,1,10,11,10,0))
    (32, 2, 10, 2, 2)
    >>> tariff.calc(datetime(2013,12,1,9,0,0), datetime(2014,11,1,10,10,0))
